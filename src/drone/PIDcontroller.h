...
class PIDcontroller {
  public:
    ...
    //Performance
    float Pgain[3] = {2.0, 2.0, 0};     //Proportional gain for roll, pitch & yaw, percentage difference per ESC at 10 degrees
    float Igain[3] = {.0000, .0000, 0}; //Integral gain for roll, pitch & yaw, changes motor performance over time, devided by 1000 due to converting Isum to seconds
    float Dgain[3] = {.33, .33, 0};     //Differential gain for roll, pitch & yaw, helps control the rotation speed
    /* Settings */
